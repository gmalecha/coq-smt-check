...
module PolyaExec : (Exec with type instance = RealInstance.instance) =
struct
  open RealInstance
  ...
  let parse_result debug inst result =
    let _ =
      debug (fun _ -> Pp.(str "Polya output" ++ fnl () ++ str result))
    in
    ...
    if Str.string_partial_match ptrn_success result 0 then
      ...
      let _ = Format.eprintf "Bad Polya output:\n%s" result in
      assert false

  let execute ~debug inst =
    let (in_channel,out_channel) = Unix.open_process "polya STDIN" in
    let _ =
      begin
...
module PolyaRealSolver = Solver.Make (Solver.RealInstance) (PolyaExec) ;;

Tactic.SmtTactic.register_smt_solver "polya" (fun _ -> PolyaRealSolver.solve)
